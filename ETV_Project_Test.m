...
max_clust_tot=500; %Maximum number of clusters across epoch
...
for i=2:400 %numFrames  %length(vali)
    tic
   % figure
    ...
    if length(idxi_x)./(size(val,1)*size(val,2))*100 <=1 % Only chose frames below 1%
    euc_mat=[];
...
for i=1:100 %1:size(cntrd_cell,1)
